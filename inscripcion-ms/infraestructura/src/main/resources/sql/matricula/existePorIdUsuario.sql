...
u.email as email_usuario, u.ciudad as ciudad_usuario, u.direccion as direccion_usuario
FROM matricula as m INNER JOIN programa as p ON (m.programa_id = p.id)
INNER JOIN usuario as u ON (m.usuario_id = u.id) WHERE m.usuario_id = :id;